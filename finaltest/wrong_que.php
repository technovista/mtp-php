...
$que=$_SESSION['totalwrong'];
$q1=(substr_replace($que," ",0,1));
//echo "after sub str".$q1;
$cnt=1;
if(isset($_GET['page']))
{
	$page=$_GET['page'];
...
while($rs=mysql_fetch_assoc($qry1,3))
{
	...
	$cnt=$cnt+1;
	//$inqry=mysql_query("select distinct ab1.answerId,ab1.choice from answerbank ab1,questionanswerrelationship qar1,customtest_log cl where ab1.answerId=qar1.answerId and qar1.questionAnswerId=cl.question_answer_id and qar1.questionId='".$rs['questionId']."'")or die(mysql_error());
	$inqry=mysql_query("select distinct ab1.answerId,ab1.choice from answerbank ab1,questionanswerrelationship qar1 where ab1.answerId=qar1.answerId and qar1.questionId='".$rs['questionId']."'") or die(mysql_error());
	...
	echo "<br>";
	$tr=mysql_num_rows($inqry1);
	$f=0;
	//$c=0;
	while($inerrs=mysql_fetch_assoc($inqry,3))
	{	
		$inqry1=mysql_query("select distinct cl1.answer_given from questionanswerrelationship qar1,customtest_log cl1 where qar1.questionId='".$rs['questionId']."' and cl1.question_answer_id=qar1.questionAnswerId and cl1.customtest_id=(select max(customtest_id) from customtest_log)")or die(mysql_error());
		while($ans=mysql_fetch_assoc($inqry1,3))
		{
			
			if($rs['questionType']=='MULTIPLE_CHOICE')
			{
					
				if($inerrs['answerId']==$ans['answer_given'])
				
					{
						$dis.='<tr><td colspan="3"><input type="radio" name="" value="'.$inerrs['answerId'].'" checked>'.$inerrs['choice'].'</td></tr>';
			...
			else
			{
				...
				if($inerrs['answerId']==$ans['answer_given'] )
				{
					$dis.='<tr><td colspan="3"><input type="checkbox" name="" value="'.$inerrs['answerId'].'" checked>'.$inerrs['choice'].'</td></tr>';
					$c=1;			
				}
				else
				{
					//echo "value dnt match".$inerrs['answerId'];
...
{
	$previous=$page-1;
	$dis.="<tr><td><a href='wrong_que.php?page=$previous'>Previous</a></td>";
}
if($page!=$total_pages)
{
	$next=$page+1;
	$dis.="<td><a href='wrong_que.php?page=$next'>Next</a></td></tr>";
